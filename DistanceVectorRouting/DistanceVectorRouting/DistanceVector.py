...
class DistanceVector(Node):
    ...
    def __init__(self, name, topolink, outgoing_links, incoming_links):
        ...
        distance_vectors = {}
        distance_vectors[self.name] = 0
        
        #for neighbor in outgoing_links:
         #   distance_vectors[neighbor.name] = neighbor.weight
    ...
    def send_initial_messages(self):
        ...
        for neighbor in self.incoming_links:
            msg = (self.name, self.name, self.distance_vectors[self.name])
            send_msg(msg, neighbor.name)
    ...
    def process_BF(self):
        ...
        for msg in self.messages:   
            if msg[1] == self.name:
                continue
            if msg[1] not in distance_vectors:
                distance_vectors[msg[1]] = self.get_outgoing_neighbor_weight(msg[0]) + msg[2]
                send_updates = True
            else:
                sln = get_outgoing_neighbor_weight(msg[0]) + msg[2]
                if distance_vectors[msg[1]] > sln:
                    distance_vectors[msg[1]] = sln
                    send_updates = True
        ...
        if send_updates: 
            for neighbor in self.incoming_links:
                for key in self.distance_vectors.keys():
                    msg = (self.name, key, self.distance_vectors[key])
                    send_msg(msg, neighbor.name)
    ...
    def log_distances(self):
        ...
        soln = ""
        for keys in self.distance_vectors:
            soln += key + str(self.distance_vectors[keys]) + ","
            
        soln[:-1]
        add_entry(self.name, soln)        
